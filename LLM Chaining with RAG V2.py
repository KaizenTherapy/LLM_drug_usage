...
workspace.default.vector_index

# COMMAND ----------


index_name="workspace.default.vector_index"
...
def get_retriever(persist_dir: str = None):
    ...
    vectorstore = DatabricksVectorSearch(
        vs_index, text_column="Medical_Condition", embedding=embedding_model, columns=["Medical_Condition", "Medicine", "Website_URL" ]
    )
...
output = generate_query_to_retrieve_context_chain.invoke({
    "messages": [
        {"role": "user", "content": "What medicine should i avoid for Transthyretin amyloid cardiomyopathy?"}
    ]
})
...
output = generate_query_to_retrieve_context_chain.invoke({
    "messages": [
        {"role": "user", "content": "What medicine should i avoid for Transthyretin amyloid cardiomyopathy?"}, 
        {"role": "assistant", "content": "Transthyretin amyloid cardiomyopathy (ATTR-CM) is a rare but serious heart muscle disease that occurs when transthyretin (TTR) proteins misfold and clump together, forming amyloid fibrils. These fibrils build up in the heart tissue, causing it to stiffen and thicken, which makes it harder for the heart to pump blood. This can lead to heart failure, shortness of breath, and swelling. ATTR-CM can also present as new or worsening conduction system disease. "}, 
        {"role": "user", "content": "What medicine should i avoid for Transthyretin amyloid cardiomyopathy?"}
    ]
})
...
irrelevant_question_chain = (
  RunnableLambda(lambda x: {"result": "As a medical application I can only help with medical conditions. Please ask a medical or health related question.", "sources": []})
)
...
question_with_history_RAG_prompt = PromptTemplate(
  input_variables= ["chat_history","context", "question"],
  template = question_with_history_RAG_str
)
...
relevant_question_chain = (
  ...
  |
  {
    "prompt": question_with_history_RAG_prompt,
    "sources": itemgetter("sources")
  }
  |
  {
...
irrelevant_question_chain = (
  RunnableLambda(lambda x: {"result": 'As a medical application I can only help with medical conditions. Please ask a medical or health related question.'})
)
...
model_name = f"{catalog}.{db}.llm_med_chat_rag"

with mlflow.start_run(run_name="llm_med_chat_rag") as run:
